return { -- Fuzzy finder telescope
  ...
  config = function()
    ...
    wk.add {
      ...
      { '<leader>sc', ts_builtin.treesitter, desc = '[S]earch [C]ode with Treesitter' },
      ...
      -- notification history from nvim-notify
      { '<leader>sn', function() require('telescope').extensions.notify.notify() end, desc = '[S]earch [N]otifications', icon = '' },
    }
  end,
}
